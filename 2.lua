...
local API_URL_REPLIT = "https://4fb4ad48-15f1-4fc1-b136-6fe244fbdc36-00-11xzf7zv4wcky.sisko.replit.dev"
local maxRetries = 3
local retryDelay = 2
...
local function getHWID()
    ...
    if success then hwid = result else hwid = (gethwid and gethwid()) or lp.UserId or "unknown_id" end
    return hwid
end
...
local function verifyKey(key)
    ...
    if not requestFunc then return false, "Executor not supported (HTTP request missing)" end
    while attempt < maxRetries do 
        attempt = attempt + 1
        local success, result = pcall(function()
            local response = requestFunc({Url = API_URL_REPLIT .. "/api/verify", Method = "POST", Headers = { ["Content-Type"] = "application/json" }, Body = HttpService:JSONEncode({key = key, hwid = hwid})})
            if response.StatusCode == 200 then return HttpService:JSONDecode(response.Body)
            elseif response.StatusCode == 503 or response.StatusCode == 504 then error("Server busy/unreachable")
            else error("Server error: " .. (response.StatusCode or "unknown")) end
        end)
        if success and result then if type(result) == "table" then return result.success, result.message or "Unknown error" end
        else if attempt < maxRetries then task.wait(retryDelay) end end
    end 
    return false, "Connection timeout after " .. maxRetries .. " attempts"
end

local key = getgenv().CheckKey
if not key or key == "" then lp:Kick("\n[RISE HUB SECURITY]\n\nNO KEY PROVIDED") return end
